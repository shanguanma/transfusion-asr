...
class PogDataset(Dataset):
    def __init__(
        ...
        s2i: Dict[str, int]=dict(),
        i2s: Dict[int, str]=dict(),
        use_text_bypass: bool = False,
    ) -> None:
    ...
    def __getitem__(self, index: int) -> Tuple[Tensor, Tensor, Tensor]:
        ...
        #print(f"type of self.s2i: {type(self.s2i)}, {self.s2i}" )
        ...
        if self.use_text_bypass:
            ...
            #wavlm_feat = torch.load(
            #    row.wavlm_path, map_location="cpu"
            #)  # (seq_len, dim)
            with open(row.wavlm_path,'rb')as f:
                arr = lilcom.decompress(f.read())
                wavlm_feat = torch.from_numpy(arr) # (seq_len, dim)
        ...
        t = torch.tensor(
            random.randint(0, self.T - 1), dtype=torch.long
        )  # sample t \in [0, T-1], aka [0, T)
...
def collate_batch(
    ...
    cond_padding_mask = (
        torch.arange(cond_emb.shape[1], dtype=torch.long)[None, :] >= ll[:, None]
    )
