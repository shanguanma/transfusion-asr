...
class MultinomialDiffusion:
    ...
    def q_posterior(self, log_x_start, log_x_t, t):
        ...
        log_EV_qxtmin_x0 = self.q_pred(
            log_x_start, t_minus_1
        )  # log( q(x_{t-1} | x_0) )
        # if t == 0, then log( q(x_0 | x_0) ) = log( one_hot(x_0) ), not even random at that point.
        # so, where t == 0
        num_axes = (1,) * (len(log_x_start.size()) - 1)
        t_broadcast = t.view(-1, *num_axes) * torch.ones_like(
            log_x_start
        )  # broadcast to non-batch axes
    ...
    def compute_Lt(
        ...
        loss = mask * decoder_nll + (1.0 - mask) * kl  # only add L0 if t == 0.
    ...
    def kl_prior(self, log_x_start: Tensor) -> Tensor:
        ...
        )  # log(1/K), broadcast to q(x_T|x_0) shape
