...
def eval_multinomial_cer(
    ...
    for i in reversed(range(0, T)):
        ...
        if mb is not None:
            mb.child.comment = f"step {i:03d}/{T:03d}"

        if plot:
            cer.append(
                1
                - (
                    (y[0, idx[idx[:, 0] == 0, 1]] == preds[0, idx[idx[:, 0] == 0, 1]])
                    .float()
                    .mean()
                )
                .cpu()
...
def _draw_denoise(
    ...
    ax.append(
        plt.axes(
            [(pad + dim[1] + 1) / (dim[1] + pad * 2), 0, 1 / (dim[1] + pad * 2), 1]
        )
    )
    ...
    for z in range(dim[0]):
        for x in range(dim[1]):
            ...
            if x > 0 and c != transitions[dim[1] - x][z] and c == label[z]:
                ...
                x > 0
                and c != transitions[dim[1] - x][z]
                and transitions[dim[1] - x][z] == label[z]
            ):
                bg[z, x] = 1
