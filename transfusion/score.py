...
class DSH:
    ...
    guidance_w: float = (
        1.5  # classifier free guidance weight [default 1.5] (Section 4.3)
    )
    enable_kevin_scaled_inference: bool = (
        True  # sequentially progressive diffusion [default True] (Section 4.2)
    )
    T_override: Union[
        None, int
    ] = 400  # allow variable transcription sizes during inference (Section 4.4)
    ...
    if set:
        ...
        if Path(f"tmp_score_cache-{name}.pt").is_file() == False:
            raise FileNotFoundError(
                "Cache file tmp_score_cache.pt not found in current directory."
            )
...
def to_text(pp, i2s: list):
    return "".join([i2s[p] for p in pp if p != 0])


# text to ids helper function
def to_inds(txt, s2i):
    return torch.tensor([s2i[ch] for ch in txt], dtype=torch.long)
...
def score_model(
    ...
    def reverse_diffusion(
        ...
        if DSH.guidance_w != 1:
            ...
            x_0_pred = (
                DSH.guidance_w * x_0_pred + (1 - DSH.guidance_w) * uncond_x_0_pred
            )
        ...
        # Equation 8a
        x_known_log = index_to_log_onehot(
            x_known, diff.num_classes, dtype=x_0_pred.dtype
        )
    ...
    if restore:
        text_targets_all, text_preds_all, resume_idx, x_0_preds = cache(
            restore=True, name=name
        )
        print(
            f"Restored eval cache containing {len(text_targets_all)} items computed by index {resume_idx}"
        )
    else:
        text_preds_all = []
        text_targets_all = []
    ...
    for i, batch in mb:
        ...
        times = get_schedule(
            cfg.model_cfg.T, jump_n_sample=DSH.jump_n_sample, jump_len=DSH.jump_len
        )
        ...
        for t_last, t_cur in progress_bar(
            zip(times[:-1], times[1:]), total=len(times) - 1, parent=mb
        ):
            if i > resume_idx:
                mb.child.comment = f"CER: {cer(text_targets, text_preds):.3f} | WER: {wer(text_targets, text_preds):.3f}"

            t = torch.ones((effective_bs,), dtype=torch.long, device=x.device) * (
                t_last
            )
            if t_cur < t_last:
        ...
        if x_0_preds is None:
            ...
            x_0_preds = torch.cat(
                (x_0_preds, x_0_pred), dim=0
            )  # (tot+bs, seq_len, vocab_size)
        ...
        if i % 25 == 0:
            ...
            mb.write(
                f"[{i:04d}/{len(dl):04d}] running cer: {running_cer:4.3f} | running wer: {running_wer:4.3f}"
            )
...
def main():
    ...
    parser.add_argument(
        "--ckpt", required=True, type=str, help="model checkpoint to use."
    )
    parser.add_argument(
        "--eval_csv",
        required=True,
        type=str,
        help="csv of audio & wavlm paths to eval on.",
    )
    ...
    parser.add_argument(
        ...
        default=1,
        type=int,
        help="number of repeated ensembles (not in paper)",
    )
    ...
    parser.add_argument(
        ...
        default=None,
        type=str,
        help="used to limit to dev-clean, dev-other...",
    )
    parser.add_argument(
        "--restore",
        action="store_true",
        default=False,
        help="restore failed eval run from progress saved in tmp_score_cache.pt",
    )
    ...
    model.eval()
    print(
        f"Model loaded with {sum([p.numel() for p in model.parameters()]):,d} parameters."
    )
    ...
    if args.uttr_path_contains is not None:
        ...
        print(
            f"trimming utterances containing {args.uttr_path_contains}: {len_pre} --> {len(df)}"
        )
    ds = PogDataset(
        df,
        vocab["s2i"],
        vocab["i2s"],
        cfg.model_cfg.T if DSH.T_override is None else DSH.T_override,
