...
def get_wavlm_feat_paths(df: pd.DataFrame, ls_path, wavlm_path) -> pd.DataFrame:
    ...
    for i, row in pb:
        ...
        targ_path = (wavlm_path / rel_path).with_suffix(".llc")
...
def main():
    ...
    valid_csv.to_csv("data/valid.csv", index=False)
    print(
        f"Saved train csv (N={len(train_csv)}) and valid csv (N={len(valid_csv)} to data/"
    )
    ...
    if args.include_test:
        ...
        print(f"Saved test csv (N={len(test_csv)}) to data/test.csv")

    # save vocab as well, fairseq style
    vocab = get_vocab(ls_df)
    i2s = {i: s for i, s in enumerate(vocab)}
    s2i = {s: i for i, s in enumerate(vocab)}
    torch.save({"i2s": i2s, "s2i": s2i}, "data/vocab.pt")
    print("Vocab: ", s2i)
