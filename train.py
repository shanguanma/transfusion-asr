...
class DistributedConfig:
    ...
    # dist_url: str= "tcp://localhost:111"
    # dist_url: str= "tcp://127.0.0.1:111"
    dist_url: str = "env://"
    # n_nodes: int = 1 # Handled by deepspeed
    n_gpus_per_node: int = 3
...
class TrainConfig:
    ...
    batch_size: int = 8
    num_workers: int = 16  # for dataloder
    ...
    max_transcript_length: int = (
        300  #  0.4% of librispeech transcripts are longer than this.
    )
...
def train(rank, cfg: TrainConfig, deepspeed_cfg: argparse.Namespace):
    ...
    if cfg.distributed.n_gpus_per_node > 1:
        deepspeed.init_distributed(
            dist_backend=cfg.distributed.dist_backend,
            init_method=cfg.distributed.dist_url,
        )
        # deepspeed.init_distributed(
        #    dist_backend="nccl", init_method="tcp://10.26.6.81:7700"
        # )
    ...
    train_ds = PogDataset(
        df=train_df,
        T=cfg.model_cfg.T,
        max_transcript_length=cfg.max_transcript_length,
        s2i=vocab["s2i"],
        i2s=vocab["i2s"],
    )
    valid_ds = PogDataset(
    ...
    #    )
    ds_log_path = cfg.checkpoint_path
